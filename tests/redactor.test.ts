...
describe("Redactor Tool Tests", () => {
  ...
  describe("Entity Detection", () => {
    it("should detect entities using the compromise engine", async () => {
      ...
      const phones = entities
        ...
        .map((e) => e.text);
      expect(phones.some((p) => p.includes("410") && p.includes("555"))).toBe(
        true,
      );
    ...
    it("should detect entities using the transformers engine", async () => {
      ...
      const snippet =
        "My name is John Doe and I live in New York. You can reach me at john.doe@example.com.";
      ...
      try {
        ...
        if (
          error.message &&
          error.message.includes('Unsupported device: "wasm"')
        ) {
          console.warn(
            "Skipping transformers test due to missing wasm support in vitest node environment",
          );
          return;
        }
        throw error;
  ...
  describe("Apply Redactions", () => {
    it("should replace text with placeholders", () => {
      const text =
        "Hello, my email is test@example.com and my phone is 555-1234.";
      const entities = [
        {
      ...
      expect(result).toBe(
        "Hello, my email is [EMAIL] and my phone is [PHONE].",
      );
    });
