...
const sampleContent = fs.readFileSync(
  path.join(__dirname, "code-formatter", "sample.md"),
  "utf8",
);
    ...
    it(`should format ${snippet.languageLabel}`, async () => {
      const langDef = LANGUAGES.find(
        (l) =>
          ...
          (l.id === "handlebars" &&
            snippet.languageLabel.includes("Handlebars")),
      );

      if (!langDef) {
        throw new Error(
          `Language definition not found for label: ${snippet.languageLabel}`,
        );
      }
      ...
      try {
        ...
        if (
          error.message &&
          error.message.includes("Cannot find package 'a' imported from")
        ) {
          console.warn(
            `Skipping ${snippet.languageLabel} due to vitest WASM import issue`,
          );
          return;
        }
        throw error;
      ...
      if (
        langDef.engine === "prettier" &&
        formatted.trim() !== snippet.code.trim()
      ) {
