...
function isAcceptedImage(file: File) {
  return (
    ...
    /\.(heic|heif|tiff?|bmp|webp|avif)$/i.test(file.name)
  );
}
...
export default function ImageMetadataTool() {
  ...
  const parseFile = useCallback(async (file: File, index: number) => {
    try {
      ...
      setFiles((prev) =>
        prev.map((entry, i) =>
          i === index ? { ...entry, metadata: meta, loading: false } : entry,
        ),
      );
    } catch (err: any) {
      setFiles((prev) =>
        prev.map((entry, i) =>
          i === index
            ? {
                ...entry,
                loading: false,
                error: err?.message ?? "Failed to parse metadata",
              }
            : entry,
        ),
      );
  ...
  function exportMetadataJson() {
    ...
    downloadBlob(
      blob,
      `${metadata.fileName.replace(/\.[^.]+$/, "")}_metadata.json`,
    );
  }
  ...
  const totalFields =
    metadata?.categories.reduce((sum, c) => sum + c.fields.length, 0) ?? 0;
    ...
    <div className="space-y-6">
      ...
      >
        ...
        <p className="mt-1.5 text-xs text-text-muted">
          JPEG, PNG, TIFF, HEIC, WebP, AVIF and more
        </p>
                  ...
                  >
                    ...
                    <div className="h-9 w-9 shrink-0 overflow-hidden rounded bg-bg-primary">
                      {entry.metadata?.previewUrl ? (
                        <img
                          src={entry.metadata.previewUrl}
                          alt={`Thumbnail of ${entry.file.name}`}
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="text-[10px] text-text-muted">
                        ...
                        {entry.error && " — error"}
                        {entry.metadata &&
                          ` — ${entry.metadata.categories.reduce((s, c) => s + c.fields.length, 0)} fields`}
                      </p>
                    </div>
                    <button
                      type="button"
              ...
              <div className="rounded-xl border border-accent-green/20 bg-accent-green/5 p-4">
                ...
                <div className="max-h-40 space-y-1.5 overflow-y-auto">
                  {strippedFiles.map((file, i) => (
                    <div
                      key={`${file.name}-${i}`}
                      className="flex items-center gap-2 rounded-lg border border-accent-green/10 bg-bg-card px-3 py-1.5"
                    >
                      <svg
          ...
          <div className="space-y-4">
            ...
            {selected?.error && (
              <div className="rounded-xl border border-accent-red/20 bg-accent-red/5 p-6 text-center">
                <svg
                  className="mx-auto mb-2 h-8 w-8 text-accent-red"
                  fill="none"
                  viewBox="0 0 24 24"
                  stroke="currentColor"
                  strokeWidth="1.5"
              ...
              <>
                ...
                <div className="rounded-xl border border-border-card bg-bg-card p-5">
                  <div className="flex items-start gap-4">
                    ...
                    <div className="min-w-0 flex-1">
                      <h3 className="font-display truncate text-sm font-bold text-text-primary">
                        {metadata.fileName}
                      </h3>
                ...
                {totalFields === 0 && (
                  <div className="rounded-xl border border-border-card bg-bg-card p-8 text-center">
                    ...
                    <p className="mt-1 text-xs text-text-muted">
                      The image may have already been stripped of metadata
                    </p>
                  </div>
                )}
                ...
                {filteredCategories.map((category) => (
                  <div
                    key={category.label}
                    className="overflow-hidden rounded-xl border border-border-card bg-bg-card"
                  >
                    <button
                      type="button"
                    ...
                    >
                      ...
                      <span className="font-display flex-1 text-sm font-semibold text-text-primary">
                        {category.label}
                      </span>
                    ...
                    {expandedCategories.has(category.label) && (
                      <div className="border-t border-border-card">
                        {category.fields.map((field) => (
                          ...
                          >
                            <span className="w-40 shrink-0 pt-0.5 text-xs font-medium text-text-secondary">
                              {field.label}
                            </span>
                            <div className="min-w-0 flex-1">
                              {editingKey === field.key ? (
                                ...
                                <div className="flex items-start gap-2">
                                  <p
                                    className={`break-all text-xs ${isFieldEdited(field) ? "text-accent-purple" : "text-text-primary"}`}
                                  >
                                    {getFieldValue(field)}
                                    {isFieldEdited(field) && (
                                      <span className="ml-1.5 text-[10px] text-accent-purple">
                                        (edited)
                                      </span>
                                    )}
                                  </p>
