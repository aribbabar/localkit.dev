...
const INPUT_EXTENSION_PATTERN =
  /\.(heic|heif|psd|tga|bmp|tiff?|ico|pcx|pgm|ppm|hdr|xcf|gif)$/i;
const ACCEPTED_INPUTS =
  "image/*,.heic,.heif,.psd,.tga,.bmp,.tiff,.ico,.pcx,.pgm,.ppm,.hdr,.xcf";
...
function isAcceptedImage(file: File) {
  return (
    file.type.startsWith("image/") || INPUT_EXTENSION_PATTERN.test(file.name)
  );
}
...
export default function ImageConverterTool() {
  ...
  const [stripMetadata, setStripMetadata] = useState(
    savedPreferences.stripMetadata,
  );
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        const nextFormats = availableFormats.map((format) => ({
          ext: format.ext,
          label: format.label,
        }));
      ...
      <div className="space-y-6">
        ...
        >
          ...
          <p className="mt-1.5 text-xs text-text-muted">
            PNG, JPG, GIF, BMP, TIFF, HEIC, PSD, ICO, TGA and more
          </p>
          ...
          <div className="rounded-xl border border-border-card bg-bg-card p-5">
            ...
            <div className="space-y-2">
              {convertedFiles.map((file, index) => {
                ...
                const delta =
                  originalSize > 0
                    ? ((outputSize - originalSize) / originalSize) * 100
                    : 0;
                  ...
                  >
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="text-[10px] text-text-muted">
                        ...
                        {(shrank || grew) && (
                          <span
                            className={`ml-1 ${shrank ? "text-accent-green" : "text-accent-orange"}`}
                          >
                            ({delta > 0 ? "+" : ""}
                            {Math.round(delta)}%)
                          </span>
