...
function evaluateStrength(opts: Options): {
  strength: Strength;
  label: string;
} {
  const pools = [
      ...
      <div className="rounded-xl border border-border-card bg-bg-card/60 p-4">
        ...
        <div className="mt-3 flex items-center gap-3">
          <div className="h-1.5 flex-1 rounded-full bg-bg-secondary overflow-hidden">
            <div
              className={`h-full rounded-full transition-all duration-300 ${colors.bar} ${strengthWidth[strength]}`}
            />
          </div>
          <span
            className={`text-xs font-medium ${colors.text} min-w-[5rem] text-right`}
          >
            {label}
          </span>
        </div>
